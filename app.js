...
//     const orderedApps = filteredApps.sort((a, b) =>
...
//   const orderedApps = filteredCategories[catKey].children.sort((a, b) =>
//     a.order > b.order ? 1 : a.order < b.order ? -1 : 0
//   );
...
//         <p class="content-black animate__animated animate__fadeIn animate__slow" style="font-size: 18px">
//           ${appInfo.description}
//         </p>
...
function destroySlider() {
  swiperContainer.classList.add("remove");
  setTimeout(() => {
    swiperContainer.remove();
  }, 500);
}

const renderSlider = (arraySlides) => {
  let slides = arraySlides.map((item) => {
    let buttonsHTML = "";
    if (item.buttons.includes("omit")) {
      buttonsHTML += `
      <button type="button" class="button secondary-button" onclick="destroySlider()">
    ...
    if (item.buttons.includes("play")) {
      buttonsHTML += `
      <button type="button" class="button primary-button" onclick="destroySlider()">
